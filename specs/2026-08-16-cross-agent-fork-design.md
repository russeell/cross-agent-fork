...
> 日期：2026-08-17 ｜ 状态：v0.1 设计定稿（极简版：整会话 fork，仅 CC↔Codex）｜ 配套：README.md、docs/PORTING.md

## 1. 背景与定位

**问题**：同时使用 Claude Code 和 Codex 的开发者，会话被各自私有格式锁死——在 A 里做到一半，无法在 B 里接着干，只能手工粘贴摘要。

**目标**：在**主流 agent 集合**内实现任意 → 任意 fork：Claude Code ↔ Codex ↔ opencode ↔ Gemini CLI ↔ Cursor ↔ Grok Build ↔ Cline ↔ Aider ↔ Kimi Code ↔ Copilot CLI（写侧按 Tier 分级承诺，见 §5.1；集合随社区反馈扩展）。

**v0.1** = 把 A 的**整个会话**变成 B 里一条**可恢复的会话线**，原会话不动；先打通 CC↔Codex 两条已验证的路（L1+L2），其余主流 agent 按时间线（§5.1）逐个加入。
...
**非目标**（v0.1 明确不做）：任意边界分叉（`--at`，v0.2）；标题继承；谱系记录/tree；工具 schema 保真；配置/权限/环境迁移；子代理/后台 fork；TUI/GUI；同步镜像；handoff 文档。
...
## 11. 仓库结构（预设计，~1100 行含测试）

```
cross-agent-fork/
├── README.md                     # 门面：定位、快速开始、使用路径
├── LICENSE                       # MIT
├── pyproject.toml                # 零运行时依赖（纯 stdlib）
├── caf/
│   ├── __init__.py               # 版本号（~5 行）
│   ├── __main__.py               # python -m caf 入口（~5 行）
...
├── plugins/
│   └── dsh.py                    # DeepSeek Harness 社区插件（zstd JSONL，~280 行）
├── skills/
│   └── caf/                      # SKILL.md + references/（markdown，零代码）
...
预估：核心逻辑 ~1100 行（含 DSH 插件），测试 ~400 行，脚本/配置 ~50 行；`skills/` 是 markdown，不计入代码量。

## 12. 使用表面

| 表面 | 形态 | 版本 |
|---|---|---|
| 终端 | CLI 本身（fork/list/doctor） | v0.1 |
| Skills | per-agent SKILL.md，agent 隐式触发「对话即入口」 | v0.1（零代码） |
| MCP | `caf mcp`，任意 MCP 客户端原生调用 | v0.2 |
...
分发：仓库内 `skills/caf/` 随 v0.1 交付；Codex 复制到 `~/.agents/skills/caf`（官方 USER 级位置）；Claude Code 走 marketplace 打包（v0.2）。
