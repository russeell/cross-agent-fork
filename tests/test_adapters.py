...
from caf.core import CafxError, SessionIR, SessionMeta, Turn
...
class ClaudeAdapterTest(unittest.TestCase):
    ...
    def test_write_roundtrip(self):
        ...
        reloaded = adapter.load_session(new_id)
...
class CodexAdapterTest(unittest.TestCase):
    def setUp(self):
        ...
        shutil.copy(
            ...
            d / "rollout-2026-08-01T10-00-00-019e0000-0000-0000-0000-000000000001.jsonl",
        )
    ...
    def test_scan_uses_threads_table(self):
        ...
        con.execute(
            ...
            "'openai', ?, ?, 'none', 'never', 1)",
            ("019e0000-0000-0000-0000-000000000001", "/tmp/fake.jsonl",
             "/tmp/fixture-proj", "来自 threads 表的标题"),
        )
        con.commit()
        con.close()
    ...
    @mock.patch("caf.adapters.codex.import_external_session", return_value="019e-fake-thread")
    def test_write_official_import(self, imp):
        adapter = CodexAdapter()
        src = Path(self.tmp) / "x.jsonl"
        src.write_text("{}", encoding="utf-8")
        ir = SessionIR(
            SessionMeta("claude", "src", project_dir="/tmp/fixture-proj",
                        source_path=str(src)),
            [],
        )
        new_id = adapter.write(ir)
    ...
    def test_write_bridges_non_cc_source(self):
        ...
        with _mock.patch("caf.adapters.codex.import_external_session",
                         return_value="019e-bridged-thread") as imp:
            ir = SessionIR(
                SessionMeta("dsh", "session-abc", title="DSH 会话",
                            project_dir="/tmp/fixture-proj"),
                [Turn(1, "user", "你好"), Turn(2, "assistant", "你好！")],
            )
            new_id = adapter.write(ir)
    ...
    def test_write_bridges_modified_ir(self):
        ...
        src.write_text("{}", encoding="utf-8")
        with _mock.patch("caf.adapters.codex.import_external_session",
                         return_value="019e-modified-thread") as imp:
            ir = SessionIR(
    ...
    def test_at_boundary_mirror_content(self):
        ...
        ir = SessionIR(
            ...
            [Turn(1, "user", "u1"), Turn(2, "assistant", "a1"),
             Turn(3, "user", "u2"), Turn(4, "assistant", "a2")],
        )
        ir.turns, _ = slice_turns(ir.turns, 1, "through")
        ir.modified = True
        with _mock.patch("caf.adapters.codex.import_external_session",
    ...
    def test_parse_import_completed(self):
        params = {"importId": "i1", "itemTypeResults": [
            {"itemType": "SESSIONS", "successes": [
                {"itemType": "SESSIONS", "cwd": "/tmp/p", "source": "x.jsonl",
                 "target": "019e-thread", "title": "OAuth 重构"},
            ], "failures": []},
        ]}
    ...
    def test_parse_import_failure(self):
        params = {"importId": "i1", "itemTypeResults": [
            {"itemType": "SESSIONS", "successes": [],
             "failures": [{"itemType": "SESSIONS", "errorType": "parse",
                           "failureStage": "ingest", "message": "bad file"}]},
        ]}
    ...
    def test_find_session_ambiguous(self):
        ...
        class TwoSessionsAdapter(Adapter):
            ...
            def scan_sessions(self):
                return [
                    SessionMeta("codex", "019f6621-31ff-7160-8c4e-8f9303746637", turns=1),
                    SessionMeta("codex", "019f6621-32af-7df0-940c-0deb280ebd89", turns=2),
                ]
